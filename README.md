...
| `waitforit-arch` | Архітектурний інструментарій для Go-сервісу `waitforit` (DDD/Hexagonal). Прив'язаний до репозиторію `waitforit`: хуки запускають `gofmt`/`go build`/`go vet`, скіли спираються на `CLAUDE.md`, `.docs/`, `src/<bc>` і `.claude/rules/<bc>.md`. | Скіли `plan-feature`, `arch-test`, `dependency-rules`, `bc-guide`; агент `feature-scout`; хуки `gofmt` (PostToolUse), `vet-before-commit` (PreToolUse) |
